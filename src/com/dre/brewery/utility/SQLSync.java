...
public class SQLSync {
	...
	public void fetchPlayerLoginData(final UUID uuid) {
		try {
			...
			P.p.getServer().getGlobalRegionScheduler().run(P.p, val -> BPlayer.sqlRemoved(uuid));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void initAsyncTask() {
		...
		sqlTaskRunning = true;
		P.p.getServer().getGlobalRegionScheduler().run(P.p, val -> new SQLSaver().run());
	}
							...
							if (d.offlineDrain) {
								...
								if (resultSet.next()) {
									...
									if (storedOfflineDrunk != d.offlineDrunk) {
										...
										P.p.getServer().getGlobalRegionScheduler().run(P.p, val-> BPlayer.sqlRemoved(d.uuid));
										continue;
									}
								}
							}
